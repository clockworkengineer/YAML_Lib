...
TEST_CASE("Check YAML Parsing of comments.", "[YAML][parse][Comment]") {
  ...
  SECTION("YAML parse of comments and one before start.", "[YAML][parse][Comment]") {
  ...
  SECTION("YAML parse of comments and three before start.", "[YAML][parse][Comment]") {
    BufferSource source{
        "# Comment 1\n# Comment 2\n# Comment 3\n---\n# Comment "
        "4\n   - 'One'\n# Comment 5\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
  ...
  SECTION("YAML parse of comments one on the same line as start.", "[YAML][parse][Comment]") {
    BufferSource source{"---# Comment 1\n   - 'One'\n# Comment 2\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    REQUIRE_FALSE(!isA<Array>(yaml.document(0)));
    REQUIRE(NRef<String>(yaml.document(0)[0]).value() == "One");
  ...
  SECTION("YAML parse of comment one same line as a string.", "[YAML][parse][Comment]") {
    BufferSource source{"---\n   - One String   # Comment \n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    REQUIRE_FALSE(!isA<Array>(yaml.document(0)));
  ...
  SECTION("YAML parse of comments one same line array elements.", "[YAML][parse][Comment]") {
    BufferSource source{
        "---\n   - One String   # Comment \n   - Two String "
