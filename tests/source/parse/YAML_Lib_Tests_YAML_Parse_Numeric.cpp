...
TEST_CASE("Check YAML Parsing of numeric types.", "[YAML][Parse][Scalar][Numeric]") {
  ...
  SECTION("YAML parse an integer.", "[YAML][Parse][Scalar][Integer]") {
    ...
    REQUIRE(YRef<Number>(yaml.document(0)).value<int>() == 4567000);
  }
  ...
  SECTION("YAML parse a floating point.", "[YAML][Parse][Scalar][Integer]") {
    ...
    REQUIRE(YRef<Number>(yaml.document(0)).value<float>() == 3.141592741f);
  }
  ...
  SECTION("YAML parse a scientific floating point.", "[YAML][Parse][Scalar][Float]") {
    ...
    REQUIRE_FALSE(!isA<Number>(yaml.document(0)));
    REQUIRE_FALSE(!YRef<Number>(yaml.document(0)).is<float>());
    REQUIRE(YRef<Number>(yaml.document(0)).value<float>() ==
  ...
  SECTION("YAML parse an hexadecimal integer.", "[YAML][Parse][Scalar][Hexadecimal]") {
    ...
    REQUIRE_FALSE(!isA<Number>(yaml.document(0)));
    REQUIRE_FALSE(!YRef<Number>(yaml.document(0)).is<int>());
    REQUIRE(YRef<Number>(yaml.document(0)).value<int>() == 0x4444);
  }
  SECTION("YAML parse an octal integer.", "[YAML][Parse][Scalar][Octal]") {
    ...
    REQUIRE_FALSE(!isA<Number>(yaml.document(0)));
    REQUIRE_FALSE(!YRef<Number>(yaml.document(0)).is<int>());
    REQUIRE(YRef<Number>(yaml.document(0)).value<int>() == 04444);
  }
  SECTION("YAML parse an integer and float strings.",
    ...
    REQUIRE(YRef<String>(yaml.document(0)[0]).value() == "4567000 String");
    REQUIRE(YRef<String>(yaml.document(0)[1]).value() == "5.666666 String");
  }
}