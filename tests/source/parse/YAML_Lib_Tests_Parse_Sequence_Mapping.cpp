...
TEST_CASE("Check YAML parse mapping between sequences.", "[YAML][Parse][Sequence Mapping]") {
  const YAML yaml;
  ...
  SECTION(
      "YAML parse mapping between sequences (two key value pairs and "
      "comments).",
      "[YAML][Parse][Sequence Mapping]]") {
    BufferSource source{
        "? # PLAY SCHEDULE\n  - Detroit Tigers\n  - Chicago Cubs\n:\n  - "
  ...
  SECTION("YAML parse mapping (strings) between sequences.", "[YAML][Parse][Sequence Mapping]]") {
  ...
  SECTION("YAML parse mapping (integers) between sequences.", "[YAML][Parse][Sequence Mapping]]") {
