...
TEST_CASE("Check YAML Parsing of Arrays.", "[YAML][Parse][Array]") {
  ...
  SECTION("YAML parse array with multiple integer elements and check result.",
    ...
    REQUIRE(NRef<Number>(yaml.document(0)[0]).value<long>() == 1);
    REQUIRE(NRef<Number>(yaml.document(0)[1]).value<long>() == 1);
    REQUIRE(NRef<Number>(yaml.document(0)[2]).value<long>() == 2);
  }
  SECTION("YAML parse array with multiple string elements and check result.",
    ...
    REQUIRE(NRef<Array>(yaml.document(0)).size() == 4);
    REQUIRE(NRef<String>(yaml.document(0)[0]).value() == "One");
    REQUIRE(NRef<String>(yaml.document(0)[1]).value() == "Two");
    REQUIRE(NRef<String>(yaml.document(0)[2]).value() == "Three");
    REQUIRE(NRef<String>(yaml.document(0)[3]).value() == "Four");
  }
  SECTION("YAML parse array with multiple integer elements and check result.",
          "[YAML][Parse][Array]") {
    BufferSource source{"---\n   - -1\n   - +2\n   - -3\n   - 4\n"};
    ...
    REQUIRE(NRef<Number>(yaml.document(0)[1]).value<int>() == 2);
  ...
  SECTION("YAML parse nested array ", "[YAML][Parse][Array]") {
    ...
    REQUIRE(NRef<Number>(yaml.document(0)[0][0]).value<int>() == 1);
    REQUIRE(NRef<Array>(yaml.document(0)[1]).size() == 3);
    REQUIRE_FALSE(!isA<Number>(yaml.document(0)[1][0]));
    REQUIRE(NRef<Number>(yaml.document(0)[1][0]).value<int>() == 4);
    REQUIRE_FALSE(!isA<Number>(yaml.document(0)[1][1]));
    REQUIRE(NRef<Number>(yaml.document(0)[1][1]).value<int>() == 5);
  ...
  SECTION("YAML parse flat array of integers and verify.",
    ...
    REQUIRE(NRef<Number>(yaml.document(0)["items"][0]).value<int>() == 1);
    REQUIRE(NRef<Number>(yaml.document(0)["items"][1]).value<int>() == 2);
    REQUIRE(NRef<Number>(yaml.document(0)["items"][2]).value<int>() == 3);
    REQUIRE(NRef<Number>(yaml.document(0)["items"][3]).value<int>() == 4);
  ...
  SECTION("YAML parse flat array of strings and verify.",
    ...
    REQUIRE(NRef<Array>(yaml.document(0)["names"]).size() == 5);
    REQUIRE(NRef<String>(yaml.document(0)["names"][0]).value() == "one");
    REQUIRE(NRef<String>(yaml.document(0)["names"][1]).value() == "two");
    REQUIRE(NRef<String>(yaml.document(0)["names"][2]).value() == "three");
    REQUIRE(NRef<String>(yaml.document(0)["names"][3]).value() == "four");
  ...
  SECTION("YAML parse flat array of booleans and verify.",
    ...
    REQUIRE_FALSE(!isA<Array>(yaml.document(0)["names"]));
    REQUIRE(NRef<Array>(yaml.document(0)["names"]).size() == 5);
    REQUIRE(NRef<Boolean>(yaml.document(0)["names"][0]).value() == true);
    REQUIRE(NRef<Boolean>(yaml.document(0)["names"][1]).value() == false);
    REQUIRE(NRef<Boolean>(yaml.document(0)["names"][2]).value() == false);
    REQUIRE(NRef<Boolean>(yaml.document(0)["names"][3]).value() == false);
    REQUIRE(NRef<Boolean>(yaml.document(0)["names"][4]).value() == true);
  }
