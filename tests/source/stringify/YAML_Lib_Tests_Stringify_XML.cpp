...
TEST_CASE("Check YAML stringification to XML of simple types.", "[YAML][Stringify][Simple][XML]") {
  ...
  SECTION(
      "Stringify a string (abcdefghijklmnopqrstuvwxyz &<>'\") to XML with "
  ...
  SECTION(R"(Stringify XML string with escapes (1-127)' to buffer and check value.)",
          "[YAML][Stringify][XML][Escapes]") {
  ...
  SECTION(R"(Stringify XML string with escapes (128-255)' to buffer and check value.)",
          "[YAML][Stringify][XML][Escapes]") {
    std::string source{R"("abcdefghijklmnopqrstuvwxyz )"};
