...
TEST_CASE("Check YAML stringification to XML of simple types.",
          "[YAML][Stringify][Simple][XML]") {
  const YAML yaml(makeStringify<XML_Stringify>());
  SECTION("Stringify a string (abcdefghijklmnopqrstuvwxyz) to XML.",
  ...
  SECTION("Stringify a string (abcdefghijklmnopqrstuvwxyz &<>'\") to XML with "
          "default character escapes.",
          "[YAML][Stringify][String][XML]") {
    BufferDestination yamlDestination;
    yaml.parse(BufferSource{R"("abcdefghijklmnopqrstuvwxyz &<>'\"")"});
    yaml.stringify(yamlDestination);
  ...
  SECTION("XML encode an string with unprintable characters "
    ...
    // Use YAML \xNN escape sequences (raw control bytes are forbidden per
    // YAML 1.2 §5.1)
  ...
  SECTION("XML encode an string with unprintable characters (1-127) ",
          "[YAML][Stringify][XML][String]") {
    // Build source using YAML \xNN escapes for control chars (raw control bytes
    // forbidden per YAML 1.2 §5.1)
  ...
  // SECTION("XML encode an string with unprintable characters (128-255) ",
  // "[YAML][Stringify][XML][String]")
  // {
  //   std::string escaped{ "\"abcdefghijklmnopqrstuvwxyz" };
  ...
  //   // 0xFFFD means no such character in UTF8 (generated for 128-255; but can
  //   use syntax \u0080 etc).
  // }
  SECTION("Stringify a boolean (true) to XML.",
          "[YAML][Stringify][Boolean][XML]") {
    BufferDestination yamlDestination;
    yaml.parse(BufferSource{"true"});
    yaml.stringify(yamlDestination);
    REQUIRE(yamlDestination.toString() ==
            R"(<?xml version="1.0" encoding="UTF-8"?><root>true</root>)");
  }
  SECTION("Stringify a boolean (false) to XML.",
          "[YAML][Stringify][Boolean][XML]") {
    BufferDestination yamlDestination;
    yaml.parse(BufferSource{"false"});
    yaml.stringify(yamlDestination);
    REQUIRE(yamlDestination.toString() ==
            R"(<?xml version="1.0" encoding="UTF-8"?><root>false</root>)");
  }
  SECTION("Stringify a integer (98345) to XML.",
          "[YAML][Stringify][Integer][XML]") {
    BufferDestination yamlDestination;
    yaml.parse(BufferSource{"98345"});
    yaml.stringify(yamlDestination);
    REQUIRE(yamlDestination.toString() ==
            R"(<?xml version="1.0" encoding="UTF-8"?><root>98345</root>)");
  }
  SECTION("Stringify a integer (-98345) to XML.",
          "[YAML][Stringify][Integer][XML]") {
    BufferDestination yamlDestination;
  ...
  SECTION("Stringify a floating point (55.6667) to XML.",
          "[YAML][Stringify][Float][XML]") {
    BufferDestination yamlDestination;
    yaml.parse(BufferSource{"55.6667"});
    yaml.stringify(yamlDestination);
    REQUIRE(yamlDestination.toString() ==
            R"(<?xml version="1.0" encoding="UTF-8"?><root>55</root>)");
  }
  SECTION("Stringify a floating point (-55.6667) to XML.",
          "[YAML][Stringify][Float][XML]") {
    BufferDestination yamlDestination;
  ...
  SECTION("Stringify an empty object to XML.",
  ...
  SECTION(
      R"(Stringify an nested array ({"City":"London","Population":[1,2,3,4,5]}) to XML.)",
  ...
  SECTION(
    ...
    BufferDestination yamlDestination;
    yaml.parse(
        BufferSource{R"([true,"Out of time",7.89043e+18,{"key":4444}])"});
    yaml.stringify(yamlDestination);
    REQUIRE(
        yamlDestination.toString() ==
  ...
  SECTION(
      R"(Stringify XML string with escapes '\u007F (non-printable ASCII)' to buffer and check value.)",
  ...
  SECTION(
      R"(Stringify XML string with escapes (1-127)' to buffer and check value.)",
      "[YAML][Stringify][XML][Escapes]") {
  ...
  SECTION(
      R"(Stringify XML string with escapes (128-255)' to buffer and check value.)",
      "[YAML][Stringify][XML][Escapes]") {
    std::string source{R"("abcdefghijklmnopqrstuvwxyz )"};
  ...
  SECTION(R"(Stringify {"Test" : [[],[],[]]} to a XML.)",
          "[YAML][Stringify][Object][XML]") {
    BufferDestination yamlDestination;
