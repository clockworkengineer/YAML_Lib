...
TEST_CASE("Check YAML stringify.", "[YAML][Stringify]") {
  ...
  SECTION("YAML Stringify document dictionary key value pair.", "[YAML][Stringify][Dictionary]") {
  ...
  SECTION("YAML Parse array with one string elements and stringify.", "[YAML][Parse][Array]") {
    BufferSource source{"---\n   - \"One\"\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml, "---\n- \"One\"\n...\n");
  }
  SECTION("YAML Parse array with multiple string elements and stringify.", "[YAML][Parse][Array]") {
    BufferSource source{"---\n  - \"One\"\n  - \"Two\"\n  - \"Three\"\n  - \"Four\"\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml,
                "---\n- \"One\"\n- \"Two\"\n"
  ...
  SECTION("YAML Stringify sequence of double quoted strings.", "[YAML][Stringify][Comments]") {
    BufferSource source{"- \"Mark McGwire\"\n- \"Sammy Sosa\"\n- \"Ken Griffey\""};
    REQUIRE_NOTHROW(yaml.parse(source));
    REQUIRE(yaml.getNumberOfDocuments() == 1);
    compareYAML(yaml, "---\n- \"Mark McGwire\"\n- \"Sammy Sosa\"\n- \"Ken Griffey\"\n...\n");
  }
  SECTION("YAML Stringify sequence of single quoted strings.", "[YAML][Stringify][Comments]") {
    BufferSource source{"- 'Mark McGwire'\n- 'Sammy Sosa'\n- 'Ken Griffey'"};
    REQUIRE_NOTHROW(yaml.parse(source));
    REQUIRE(yaml.getNumberOfDocuments() == 1);
    compareYAML(yaml, "---\n- 'Mark McGwire'\n- 'Sammy Sosa'\n- 'Ken Griffey'\n...\n");
  }
  SECTION("YAML Stringify sequence of unquoted strings.", "[YAML][Stringify][Comments]") {
    BufferSource source{"- Mark McGwire\n- Sammy Sosa\n- Ken Griffey"};
    REQUIRE_NOTHROW(yaml.parse(source));
    REQUIRE(yaml.getNumberOfDocuments() == 1);
    compareYAML(yaml, "---\n- Mark McGwire\n- Sammy Sosa\n- Ken Griffey\n...\n");
  }
  SECTION("YAML Stringify nested array in dictionary.", "[YAML][Stringify][Comples]") {
    BufferSource source{
        "---\nhr:\n  - Mark McGwire\n  - Sammy Sosa\nrbi:\n  - "
        "Sammy Sosa\n  - Ken Griffey"};
    REQUIRE_NOTHROW(yaml.parse(source));
  ...
  SECTION("YAML Stringify block/piped strings.", "[YAML][Stringify][Comments]") {
    BufferSource source{
        "---\nname:  Mark McGwire\naccomplishment: >\n  Mark set a major "
  ...
  SECTION("YAML Stringify literal string preserves newlines.", "[YAML][Stringify][literals]") {
    BufferSource source{"--- |\n  \\//||\\/||\n  // ||  ||__\n..."};
    REQUIRE_NOTHROW(yaml.parse(source));
    REQUIRE(NRef<String>(yaml.document(0)).value() == "\\//||\\/||\n// ||  ||__");
    compareYAML(yaml, "---|\n\\//||\\/||\n// ||  ||__\n...\n");
  }
  SECTION("YAML Stringify folded newlines preserved for indented and blank lines.",
          "[YAML][Stringify][literals]") {
    BufferSource source{
  ...
  SECTION("YAML stringify various quoted scalars", "[YAML][Parse][Quoted Scalars]") {
    BufferSource source{
  ...
  SECTION("YAML parse a boolean (True) and stringify.", "[YAML][Stringify][Boolean]") {
    BufferSource source{"---\nTrue\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml, "---\nTrue\n...\n");
  }
  SECTION("YAML parse a boolean (False) and stringify.", "[YAML][Stringify][Boolean]") {
    BufferSource source{"---\nFalse\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml, "---\nFalse\n...\n");
  }
  SECTION("YAML parse a boolean (On) and stringify.", "[YAML][Stringify][Boolean]") {
    BufferSource source{"---\nOn\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml, "---\nOn\n...\n");
  }
  SECTION("YAML parse a boolean (Off) and stringify.", "[YAML][Stringify][Boolean]") {
    BufferSource source{"---\nOff\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml, "---\nOff\n...\n");
  }
  SECTION("YAML parse a boolean (Yes) and stringify.", "[YAML][Stringify][Boolean]") {
    BufferSource source{"---\nYes\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml, "---\nYes\n...\n");
  }
  SECTION("YAML parse a boolean (No) and stringify.", "[YAML][Stringify][Boolean]") {
    BufferSource source{"---\nNo\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    compareYAML(yaml, "---\nNo\n...\n");
  }
...
TEST_CASE("Check YAML stringify preserves explicit tags (gap 3.6).", "[YAML][Stringify][Tags]") {
  const YAML yaml;
  ...
  SECTION("Stringify round-trip: !!int tag preserved.", "[YAML][Stringify][Tags][Int]") {
    BufferSource source{"---\n!!int 42\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    BufferDestination dest;
    REQUIRE_NOTHROW(yaml.stringify(dest));
  ...
  SECTION("Stringify round-trip: !!bool tag preserved.", "[YAML][Stringify][Tags][Bool]") {
    BufferSource source{"---\n!!bool true\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    BufferDestination dest;
    REQUIRE_NOTHROW(yaml.stringify(dest));
  ...
  SECTION("Stringify round-trip: !!null tag preserved.", "[YAML][Stringify][Tags][Null]") {
  ...
  SECTION("Stringify round-trip: !!float tag preserved.", "[YAML][Stringify][Tags][Float]") {
    BufferSource source{"---\n!!float 3.14\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    BufferDestination dest;
    REQUIRE_NOTHROW(yaml.stringify(dest));
  ...
  SECTION("Stringify round-trip: custom !tag preserved.", "[YAML][Stringify][Tags][Custom]") {
    BufferSource source{"---\n!mytag some value\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    BufferDestination dest;
    REQUIRE_NOTHROW(yaml.stringify(dest));
  ...
  SECTION("Stringify round-trip: !!timestamp tag preserved.",
    ...
    BufferDestination dest;
    REQUIRE_NOTHROW(yaml.stringify(dest));
    REQUIRE(dest.toString().find("!!timestamp 2001-07-08") != std::string::npos);
  }
}
...
TEST_CASE("Check YAML stringify emits correct block scalar style (gap 3.7).",
  ...
  SECTION("Folded block string (>) stringifies as '>'.", "[YAML][Stringify][BlockScalar][Folded]") {
    BufferSource source{"---\ntext: >\n  hello world\n"};
    REQUIRE_NOTHROW(yaml.parse(source));
    BufferDestination dest;
    REQUIRE_NOTHROW(yaml.stringify(dest));
