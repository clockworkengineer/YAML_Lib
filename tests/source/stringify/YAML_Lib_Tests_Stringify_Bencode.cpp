...
TEST_CASE("Check YAML stringification to Bencode of simple types.",
          "[YAML][Stringify][Simple][Bencode]") {
  const YAML yaml(makeStringify<Bencode_Stringify>());
  SECTION("Stringify a string (abcdefghijklmnopqrstuvwxyz) to Bencode.",
          "[YAML][Stringify][String][Bencode]") {
  ...
  SECTION("Stringify a floating point (-55.6667) to Bencode.",
          "[YAML][Stringify][Float][Bencode]") {
    BufferDestination yamlDestination;
    yaml.parse(BufferSource{"-55.6667"});
    yaml.stringify(yamlDestination);
  ...
  SECTION(
      R"(Stringify a nested object ([true,"Out of time",7.89043e+18,{"key":4444}]) to a Bencode.)",
    ...
    REQUIRE(yamlDestination.toString() ==
            "l4:True11:Out of timei7890430242211233792ed3:keyi4444eee");
  }
  ...
  SECTION(R"(Stringify {"Test" : [[],[],[]]} to a Bencode.)",
          "[YAML][Stringify][Object][Bencode]") {
    BufferDestination yamlDestination;
