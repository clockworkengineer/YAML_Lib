...
TEST_CASE("Checks for toFile() api.", "[YAML][ToFile]") {
  SECTION("Check that toFile() works with UTF8.", "[YAML][ToFile][UTF8]") {
    ...
    YAML::toFile(testFileName, expected);
  ...
  SECTION("Check that toFile() works with UTF8BOM.",
    ...
    std::string expected{
        ...
        "MYSQL_ROOT_PASSWORD: somewordpress\n    MYSQL_DATABASE: wordpress\n   "
        " MYSQL_USER: wordpress\n    MYSQL_PASSWORD: wordpress\n"};
    YAML::toFile(testFileName, expected, YAML::Format::utf8BOM);
    REQUIRE(YAML::fromFile(testFileName) == expected);
    std::filesystem::remove(testFileName);
  }
  SECTION("Check that toFile() works with UTF16BE.",
    ...
    std::string expected{
        ...
        "MYSQL_ROOT_PASSWORD: somewordpress\n    MYSQL_DATABASE: wordpress\n   "
        " MYSQL_USER: wordpress\n    MYSQL_PASSWORD: wordpress\n"};
    YAML::toFile(testFileName, expected, YAML::Format::utf16BE);
    REQUIRE(YAML::fromFile(testFileName) == expected);
    std::filesystem::remove(testFileName);
  }
  ...
  // "[YAML][ToFile][UTF32BE]")
  // {
  //     std::string testFileName{ prefixPath(kGeneratedYAMLFile) };
  //     std::string expected{ R"([true  , "Out of time",  7.89043e+18, true])"
  //     }; REQUIRE_THROWS_WITH(YAML::toFile(testFileName, expected,
  //     YAML::Format::utf32BE),
