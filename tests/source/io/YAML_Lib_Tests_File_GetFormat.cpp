...
TEST_CASE("Checks for getFileFormat() api.", "[YAML][GetFileFormat]")
{
    ...
    {
        REQUIRE(YAML::getFileFormat(prefixPath("testfile033.yaml")) == YAML::Format::utf8);
    }
    SECTION("Check that getFileFormat() works with UTF8BOM.", "[YAML][GetFileFormat][UTF8BOM]")
    {
        REQUIRE(YAML::getFileFormat(prefixPath("testfile034.yaml")) == YAML::Format::utf8BOM);
    }
    SECTION("Check that getFileFormat() works with UTF16BE.", "[YAML][GetFileFormat][UTF16BE]")
    {
        REQUIRE(YAML::getFileFormat(prefixPath("testfile035.yaml")) == YAML::Format::utf16BE);
    }
    SECTION("Check that getFileFormat() works with UTF16LE.", "[YAML][GetFileFormat][UTF16LE]")
    {
        REQUIRE(YAML::getFileFormat(prefixPath("testfile036.yaml")) == YAML::Format::utf16LE);
    }
