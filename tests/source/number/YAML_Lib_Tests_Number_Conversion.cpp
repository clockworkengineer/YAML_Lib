...
TEST_CASE("Check JNode Number conversion.", "[YAML][JNode][Number]")
{
    ...
    SECTION("Float converted to int.", "[YAML][JNode][Number][Float]")
    {
        ...
        REQUIRE(NRef<Number>(yaml.document(0)).value<int>() == 6788889);
    }
    SECTION("Float converted to long.", "[YAML][JNode][Number][Float]")
    {
        ...
        yaml.parse(yamlSource);
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<float>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<long>());
        REQUIRE(NRef<Number>(yaml.document(0)).value<long>() == 6788889);
    }
    SECTION("Float converted to long long.", "[YAML][JNode][Number][Float]")
    {
        ...
        yaml.parse(yamlSource);
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<float>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<long long>());
        REQUIRE(NRef<Number>(yaml.document(0)).value<long long>() == 6788889ll);
    }
    SECTION("Float converted to double.", "[YAML][JNode][Number][Float]")
    {
        ...
        yaml.parse(yamlSource);
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<float>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<double>());
        REQUIRE_FALSE(!equalFloatingPoint(NRef<Number>(yaml.document(0)).value<double>(), 678.8990, 0.0001));
    }
    SECTION("Float converted to long double.", "[YAML][JNode][Number][Float]")
    {
        ...
        yaml.parse(yamlSource);
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<float>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<long double>());
        REQUIRE_FALSE(!equalFloatingPoint(NRef<Number>(yaml.document(0)).value<long double>(), 678.8990l, 0.0001));
    }
    SECTION("Int converted to long.", "[YAML][JNode][Number][Int]")
    {
        ...
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<int>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<long>());
        REQUIRE(NRef<Number>(yaml.document(0)).value<long>() == 78989);
    }
    SECTION("Int converted to long long.", "[YAML][JNode][Number][Int]")
    {
        ...
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<int>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<long long>());
        REQUIRE(NRef<Number>(yaml.document(0)).value<long long>() == 78989ll);
    }
    SECTION("Int converted to float.", "[YAML][JNode][Number][Int]")
    {
        ...
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<int>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<float>());
        REQUIRE_FALSE(!equalFloatingPoint(NRef<Number>(yaml.document(0)).value<float>(), 78989.0f, 0.0001));
    }
    SECTION("Int converted to double.", "[YAML][JNode][Number][Int]")
    {
        ...
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<int>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<double>());
        REQUIRE_FALSE(!equalFloatingPoint(NRef<Number>(yaml.document(0)).value<double>(), 78989.0, 0.0001));
    }
    SECTION("Int converted to long double.", "[YAML][JNode][Number][Int]")
    {
        ...
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<int>());
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<long double>());
        REQUIRE_FALSE(!equalFloatingPoint(NRef<Number>(yaml.document(0)).value<long double>(), 78989.0l, 0.0001));
    }
    SECTION("Large positive integer conversion.", "[YAML][JNode][Number][Long Long]")
    {
        ...
        REQUIRE(NRef<Number>(yaml.document(0)).value<long long>() == 9223372036854775807ll);
    }
    SECTION("Large negative integer conversion.", "[YAML][JNode][Number][Long Long]")
    {
        ...
        REQUIRE(NRef<Number>(yaml.document(0)).value<long long>() == -877994604561387500ll);
    }
    SECTION("Positive integer overflow conversion.", "[YAML][JNode][Number][Long Long]")
    {
        ...
        REQUIRE_FALSE(NRef<Number>(yaml.document(0)).is<long long>());
        REQUIRE_FALSE(!NRef<Number>(yaml.document(0)).is<float>());
        REQUIRE_FALSE(!equalFloatingPoint(NRef<Number>(yaml.document(0)).value<float>(), 9223372036854775808.0f, 0.0001));
    }
