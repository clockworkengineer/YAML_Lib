...
TEST_CASE("Check Node invalid number conversion.", "[YAML][Node][Number][Invalid]") {
  const YAML yaml;
  SECTION("Check integer with invalid characters.", "[YAML][Node][Number][Invalid]") {
  ...
  SECTION("Check floating point with invalid exponent.", "[YAML][Node][Number][Invalid]") {
    BufferSource yamlSource{"78.e43e-2"};
    REQUIRE_NOTHROW(yaml.parse(yamlSource));
    REQUIRE_FALSE(!isA<String>(yaml.document(0)));
    REQUIRE(NRef<String>(yaml.document(0)).value() == "78.e43e-2");
  }
  SECTION("Check floating point with multiple decimal points.", "[YAML][Node][Number][Invalid]") {
...
TEST_CASE("Check Node Number API(s) for all supported number types.", "[YAML][Node][Number]") {
  YAML yaml;
  SECTION("Check numbers are the correct type.", "[YAML][Node][Number][Addition]") {
  ...
  SECTION("Simple arithmetic add one to a number.", "[YAML][Node][Number][Get/Set]") {
