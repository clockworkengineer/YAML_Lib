...
struct CountingResource : std::pmr::memory_resource {
 ...
 private:
  void* do_allocate(std::size_t bytesRequested, std::size_t alignment) override {
    ++allocations;
    bytes += bytesRequested;
    return upstream->allocate(bytesRequested, alignment);
  }

  void do_deallocate(void* p, std::size_t bytesRequested, std::size_t alignment) override {
...
TEST_CASE("YAML::Options supports strict boolean parsing and memory resources",
          "[YAML][Options][API]") {
  Options options;
...
TEST_CASE("YAML::Options supports custom parser and stringifier implementations",
          "[YAML][Options][Customize]") {
  Options options;
...
TEST_CASE("Error handler registration is preserved for no-exceptions builds",
          "[YAML][NoExceptions]") {
