...
root: [*a, *a, *a, *a, *a]
  ...
  REQUIRE_THROWS_WITH(yaml.parse(BufferSource{maliciousYaml}),
                      Catch::Matchers::ContainsSubstring("YAML alias expansion limit exceeded"));
}
...
TEST_CASE("Parser rejects deeply nested YAML before stack exhaustion",
          "[YAML][Security][DepthLimit]") {
  ::YAML_Lib::Options options;
  ...
  REQUIRE_THROWS_WITH(yaml.parse(BufferSource{deepYaml}),
                      Catch::Matchers::ContainsSubstring("nesting depth limit"));
}

TEST_CASE("Parser rejects too many documents in untrusted input",
          "[YAML][Security][DocumentLimit]") {
  ::YAML_Lib::Options options;
...
value: second
  ...
  REQUIRE_THROWS_WITH(
      yaml.parse(BufferSource{multiDocYaml}),
      Catch::Matchers::ContainsSubstring("document count exceeds configured limit"));
}
