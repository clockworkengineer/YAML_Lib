...
TEST_CASE("Check use of Node assigment operators.", "[YAML][Node][Assignment]") {
  ...
  SECTION("Assign long double to Node.", "[YAML][Node][Assignment][Long Double]") {
    ...
    REQUIRE_FALSE(
        !equalFloatingPoint(NRef<Number>(yNode).value<long double>(), 66666.8888l, 0.0001));
  }
  ...
  SECTION("Assign Node with object with a nested object.",
          "[YAML][Node][Constructor][Dictionary]") {
