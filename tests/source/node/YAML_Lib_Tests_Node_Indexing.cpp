...
TEST_CASE("Check use of Node indexing operators.", "[YAML][Node][Index]") {
  ...
  SECTION("Parse array with embedded dictionary and check its components using indexing.",
          "[YAML][Node][Index]") {
  ...
  SECTION("Parse dictionary and check an invalid key generates exception.", "[YAML][Node][Index]") {
    ...
    REQUIRE_THROWS_WITH(isA<Dictionary>(yaml.document(0)["Cityy"]),
                        "Node Error: Invalid key used to access dictionary.");
  }
  SECTION("Parse array and check an invalid index generates exception.", "[YAML][Node][Index]") {
    ...
    REQUIRE_THROWS_WITH(isA<Array>(yaml.document(0)[3]),
                        "Node Error: Invalid index used to access array.");
  }
}
