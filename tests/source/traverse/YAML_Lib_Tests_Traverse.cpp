...
class YAML_Analyzer : public IAction
{
public:
  YAML_Analyzer() = default;
  virtual ~YAML_Analyzer() = default;
  // Add YNode details to analysis
  virtual void onYNode([[maybe_unused]] const YNode &yNode) override { totalNodes++; }
  // Add string details to analysis
  virtual void onString([[maybe_unused]] const YNode &yNode) override { totalStrings++; }
  // Add number details to analysis
  virtual void onNumber(const YNode &yNode) override
  {
    const Number &yNodeNumber = YRef<Number>(yNode);
    totalNumbers++;
  ...
  virtual void onBoolean([[maybe_unused]] const YNode &yNode) override { totalBoolean++; }
  // Add null details to analysis
  virtual void onNull([[maybe_unused]] const YNode &yNode) override { totalNull++; }
  // Add array details to analysis
  virtual void onArray([[maybe_unused]] const YNode &yNode) override { totalArrays++; }
  // Add object details to analysis
  virtual void onDictionary([[maybe_unused]] const YNode &yNode) override { totalDictionarys++; }
...
TEST_CASE("YAML YNode tree traverse tests ", "[YAML][Traverse]")
{
  ...
  SECTION("Parse an Long (-2147483648) and traverse", "[YAML][Traverse][Long]")
  {
  ...
  SECTION("Parse an LongLong (2147483648) and traverse", "[YAML][Traverse][LongLong]")
  {
  ...
  SECTION("Parse an LongLong (-2147483649) and traverse", "[YAML][Traverse][LongLong]")
  {
  ...
  SECTION("Parse an double (3.402823466e+39) and traverse", "[YAML][Traverse][Double]")
  {
    BufferSource source{ "3.402823466e+39" };
    yaml.parse(source);// sizeof(doble) == sizeof(long double)
    YAML_Analyzer analyzer;
    yaml.traverse(analyzer);
    REQUIRE(analyzer.totalNodes == 1);
  ...
  SECTION("Parse an long double (1.7976931348623158e+308) and traverse", "[YAML][Traverse][LongDouble]")
  {
  ...
  SECTION("Parse a string (\"test\") and traverse", "[YAML][Traverse][String]")
  {
  ...
  SECTION("Parse a object({\"one\" : 1, \"two\" : 2, \"three\" : 3 })) and traverse", "[YAML][Traverse][Object]")
  {
  ...
  SECTION(
    "Parse a nested array ({\"City\":\"London\",\"Population\":[1,2,3,4,5]}) and traverse", "[YAML][Traverse][Array]")
  {
  ...
  SECTION("Parse a nested object ([True,\"Out of time\",7.89043e+18,{\"key\":4444}]) and traverse",
