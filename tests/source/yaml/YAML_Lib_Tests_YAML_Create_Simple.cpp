...
TEST_CASE("Check YAML object creation api.", "[YAML][Create][Dictionary]") {
  ...
  SECTION(
    ...
    REQUIRE_FALSE(!equalFloatingPoint(
        YRef<Number>(yaml.document(0)[0]["pi"]).value<float>(), 3.141f,
        0.0001));
  }
  SECTION(
    ...
    YAML yaml;
    yaml["integer"] = 300;
    REQUIRE_FALSE(!isA<Number>(yaml.document(0)[0]["integer"]));
    REQUIRE(YRef<Number>(yaml.document(0)[0]["integer"]).value<int>() == 300);
  }
  SECTION("Initialise root YAML Dictionary with one entry containing a long.",
    ...
    REQUIRE(YRef<Number>(yaml.document(0)[0]["integer"]).value<long>() ==
            30000);
  }
  ...
  SECTION("Initialise root YAML Dictionary with one entry containing a double.",
    ...
    REQUIRE_FALSE(!isA<Number>(yaml["pi"]));
    REQUIRE_FALSE(!equalFloatingPoint(
        YRef<Number>(yaml.document(0)[0]["pi"]).value<double>(), 3.141,
        0.0001));
  }
  SECTION("Initialise root YAML Dictionary with one entry containing a const "
    ...
    REQUIRE(YRef<String>(yaml.document(0)[0]["name"]).value() == "robert");
  }
  ...
  SECTION(
    ...
    REQUIRE_FALSE(!YRef<Boolean>(yaml.document(0)[0]["flag"]).value());
  }
  SECTION("Initialise root YAML Dictionary with one entry containing a null.",
    ...
    REQUIRE(YRef<Null>(yaml.document(0)[0]["nothing"]).value() == nullptr);
  }
  SECTION("Create two level object and null at base.",
    ...
    REQUIRE(YRef<Null>(yaml.document(0)[0]["nothing"]["extra"]).value() ==
            nullptr);
  }
  SECTION("Create three level object and null at base and stringify.",
    ...
    REQUIRE(
        YRef<Null>(yaml.document(0)[0]["nothing"]["extra"]["more"]).value() ==
        nullptr);
