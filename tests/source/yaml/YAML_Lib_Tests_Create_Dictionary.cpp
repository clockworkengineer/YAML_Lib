...
TEST_CASE("Check YAML object creation api.", "[YAML][Create][Dictionary]") {
  ...
  SECTION("Initialise YAML with YAML object passed to constructor and validate.",
    ...
    REQUIRE_FALSE(
        !equalFloatingPoint(NRef<Number>(yaml.document(0)["pi"]).value<float>(), 3.141f, 0.0001));
  }
  SECTION("Initialise root YAML Dictionary with one entry containing a integer.",
          "[YAML][Create][Dictionary][Number]") {
    YAML yaml;
    yaml["integer"] = 300;
    REQUIRE_FALSE(!isA<Number>(yaml.document(0)["integer"]));
    REQUIRE(NRef<Number>(yaml.document(0)["integer"]).value<int>() == 300);
  }
  SECTION("Initialise root YAML Dictionary with one entry containing a long.",
    ...
    REQUIRE(NRef<Number>(yaml.document(0)["integer"]).value<long>() == 30000);
  }
  ...
  SECTION("Initialise root YAML Dictionary with one entry containing a double.",
    ...
    REQUIRE_FALSE(!isA<Number>(yaml["pi"]));
    REQUIRE_FALSE(
        !equalFloatingPoint(NRef<Number>(yaml.document(0)["pi"]).value<double>(), 3.141, 0.0001));
  }
  SECTION(
      "Initialise root YAML Dictionary with one entry containing a const "
      "char *.",
      "[YAML][Create][Dictionary][String]") {
    YAML yaml;
    yaml["name"] = "robert";
    REQUIRE_FALSE(!isA<String>(yaml["name"]));
    REQUIRE(NRef<String>(yaml.document(0)["name"]).value() == "robert");
  }
  SECTION(
      "Initialise root YAML Dictionary with one entry containing a "
      "std::string.",
      "[YAML][Create][Dictionary][String]") {
    YAML yaml;
  ...
  SECTION("Initialise root YAML Dictionary with one entry containing a boolean.",
          "[YAML][Create][Dictionary][Boolean]") {
    YAML yaml;
    yaml["flag"] = true;
    REQUIRE_FALSE(!isA<Boolean>(yaml["flag"]));
    REQUIRE_FALSE(!NRef<Boolean>(yaml.document(0)["flag"]).value());
  }
  SECTION("Initialise root YAML Dictionary with one entry containing a null.",
    ...
    REQUIRE(NRef<Null>(yaml.document(0)["nothing"]).value() == nullptr);
  }
  SECTION("Create two level object and null at base.", "[YAML][Create][Dictionary][null]") {
    YAML yaml;
    yaml["nothing"]["extra"] = nullptr;
    REQUIRE_FALSE(!isA<Null>(yaml["nothing"]["extra"]));
    REQUIRE(NRef<Null>(yaml.document(0)["nothing"]["extra"]).value() == nullptr);
  }
  SECTION("Create three level object and null at base and stringify.",
    ...
    REQUIRE(NRef<Null>(yaml.document(0)["nothing"]["extra"]["more"]).value() == nullptr);
