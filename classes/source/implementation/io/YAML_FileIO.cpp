...
void writeYAMLString(std::ofstream& yamlFile, const std::u16string& yamlString,
  ...
  if (format == YAML::Format::utf16BE) {
    ...
    YAML_THROW(
        Error,
        "Unsupported YAML file format (Byte Order Mark) specified in call to writeYAMLString().");
  }
}
...
std::u16string readYAMLString(std::ifstream& yamlFile, const YAML::Format format) {
  ...
  if (format == YAML::Format::utf16BE) {
    while (true) {
      ...
      const char16_t ch16 = static_cast<char16_t>((static_cast<unsigned char>(hi) << 8) |
    ...
    while (true) {
      ...
      const char16_t ch16 = static_cast<char16_t>(static_cast<unsigned char>(lo) |
    ...
    YAML_THROW(
        Error,
        "Unsupported YAML file format (Byte Order Mark) specified in call to readYAMLString().");
  }
  return utf16String;
}
...
YAML::Format YAML_FileReader::getFileFormat(const std::string_view& fileName) {
  ...
  yamlFile.close();
  if (byteOrderMark == 0x2B2F7638 || byteOrderMark == 0xF7BBBF00 || byteOrderMark == 0xDD736673 ||
      byteOrderMark == 0xFBEE2800 || byteOrderMark == 0x84319533) {
    YAML_THROW(Error, "Unsupported YAML file format (Byte Order Mark) encountered.");
  }
  if (byteOrderMark == 0x0000FEFF) {
    return YAML::Format::utf32BE;
  }
  if (byteOrderMark == 0xFFFE0000) {
    return YAML::Format::utf32LE;
  }
  if ((byteOrderMark & 0xFFFFFF00) == 0xEFBBBF00) {
    return YAML::Format::utf8BOM;
  }
  if ((byteOrderMark & 0xFFFF0000) == 0xFEFF0000) {
    return YAML::Format::utf16BE;
  }
  if ((byteOrderMark & 0xFFFF0000) == 0xFFFE0000) {
    return YAML::Format::utf16LE;
  }
...
void YAML_FileWriter::write(const std::string_view& fileName, const std::string_view& yamlString,
                            const YAML::Format format) {
  const std::string fileNameStr{fileName};
