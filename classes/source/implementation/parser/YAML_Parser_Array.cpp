...
Node Default_Parser::parseArray(ISource& source, const Delimiters& delimiters,
  ...
  if (isArray(source) && ctx_.arrayIndentLevel == 0 && arrayIndent > source.getPosition().second) {
...
/// <param name="indentation">Parent indentation.</param>
/// <returns>Array Node.</returns>
Node Default_Parser::parseInlineArray(ISource& source,
                                      [[maybe_unused]] const Delimiters& delimiters,
                                      const unsigned long indentation) {
  const auto inLineArrayDelimiters = withExtras(delimiters, {kComma, kRightSquareBracket});
  auto arrayNode = Node::make<Array>();
  auto& yamlArray = NRef<Array>(arrayNode);
  {
    ...
    do {
      ...
      if (source.current() == '#') {
        YAML_THROW_POS(source,
                       "Comment must be separated from comma by whitespace in flow sequence.");
      }
      if (ctx_.inlineArrayDepth == 1 && source.more() && source.current() == kLineFeed &&
      ...
      if (!yamlArray.value().empty()) {
        ...
        if (isA<String>(element) && NRef<String>(element).value() == "-" &&
