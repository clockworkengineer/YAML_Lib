...
static YNode typeToYNode(const YAML::InitializerListTypes &type) {
  ...
  if ([[maybe_unused]] auto pValue = std::get_if<std::nullptr_t>(&type)) {
    return YNode(nullptr);
  }
  if (const auto pValue = std::get_if<YNode>(&type)) {
    return std::move(*const_cast<YNode *>(pValue));
  }
...
inline YNode::YNode(const YAML::ArrayInitializer &array) {
  *this = make<Array>();
  for (const auto &entry : array) {
    YRef<Array>(*this).add(typeToYNode(entry));
  }
}
...
inline YNode::YNode(const YAML::DictionaryInitializer &dictionary) {
  *this = make<Dictionary>();
  for (const auto &[fst, snd] : dictionary) {
    YRef<Dictionary>(*this).add(Dictionary::Entry(fst, typeToYNode(snd)));
  }
}
