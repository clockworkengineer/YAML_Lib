...
struct ObjectEntry
{
  ObjectEntry(const std::string &key, YNode &yNode) : key(YNode::make<String>(key)), yNode(std::move(yNode)) {}
  ObjectEntry(const std::string &key, YNode &&yNode) : key(YNode::make<String>(key)), yNode(std::move(yNode)) {}
...
struct Object : Variant
{
  ...
  ~Object() = default;
  // Add Entry to Object
  template<typename T> void add(T &&entry) { yNodeObject.emplace_back(std::forward<T>(entry)); }
  ...
  [[nodiscard]] int size() const { return static_cast<int>(yNodeObject.size()); }
  // Return object entry for a given key
  YNode &operator[](const std::string &key) { return findKey(yNodeObject, key)->getYNode(); }
  const YNode &operator[](const std::string &key) const { return findKey(yNodeObject, key)->getYNode(); }
