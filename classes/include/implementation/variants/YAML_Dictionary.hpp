...
struct Dictionary {
  ...
  [[nodiscard]] int size() const { return static_cast<int>(yNodeDictionary.size()); }
  // Return dictionary entry for a given key
  Node& operator[](const std::string_view& key) { return findKey(key)->getNode(); }
  const Node& operator[](const std::string_view& key) const { return findKey(key)->getNode(); }
