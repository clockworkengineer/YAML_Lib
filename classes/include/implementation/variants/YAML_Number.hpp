...
struct Number : Variant
{
  ...
  template<typename T> [[nodiscard]] bool is() const { return std::get_if<T>(&yNodeNumber) != nullptr; }
...
{
  if (const auto pValue = std::get_if<int>(&yNodeNumber)) { return convertTo<T>(*pValue); }
  if (const auto pValue = std::get_if<long>(&yNodeNumber)) { return convertTo<T>(*pValue); }
  if (const auto pValue = std::get_if<long long>(&yNodeNumber)) { return convertTo<T>(*pValue); }
  if (const auto pValue = std::get_if<float>(&yNodeNumber)) { return convertTo<T>(*pValue); }
  if (const auto pValue = std::get_if<double>(&yNodeNumber)) { return convertTo<T>(*pValue); }
  if (const auto pValue = std::get_if<long double>(&yNodeNumber)) { return convertTo<T>(*pValue); }
