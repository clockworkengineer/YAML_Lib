...
struct Boolean {
  ...
  static constexpr const char* kForms[12] = {"True",  "On",  "Yes", "true",  "yes", "on",
                                             "False", "Off", "No",  "false", "no",  "off"};

  // Constructors/Destructors
  Boolean() = default;
  Boolean(const bool boolean, const std::string_view& value) : yNodeBoolean(boolean) {
  ...
  [[nodiscard]] std::string toKey() const { return yNodeBoolean ? "true" : "false"; }
  // Valid values for true/false
  // YAML 1.1 capitalised forms and YAML 1.2 lowercase canonical forms.
  inline static const std::set<std::string_view> isTrue{"True", "On", "Yes", "true", "yes", "on"};
