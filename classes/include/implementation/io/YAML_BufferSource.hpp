...
class BufferSource final : public BufferedSourceBase {
 public:
  ...
  explicit BufferSource(T&& owned) : ownedBuffer(std::move(owned)), bufferView(ownedBuffer) {}
  BufferSource() = delete;
  ...
  [[nodiscard]] bool more() const override { return bufferPosition < bufferView.size(); }
