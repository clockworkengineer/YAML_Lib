...
class DocumentStore {
 public:
  ...
  [[nodiscard]] bool empty() const noexcept { return documents.empty(); }

  void clear() noexcept { documents.clear(); }

  void setDocuments(std::vector<Node> newDocs) { documents = std::move(newDocs); }

  void addDocument(Node doc) { documents.push_back(std::move(doc)); }

  [[nodiscard]] std::vector<Node>& getDocuments() noexcept { return documents; }

  [[nodiscard]] const std::vector<Node>& getDocuments() const noexcept { return documents; }
