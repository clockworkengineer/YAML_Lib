...
template <typename DocumentHandler, typename NumberHandler, typename StringHandler,
          typename BooleanHandler, typename NullHandler, typename TimestampHandler,
          typename DictionaryHandler, typename ArrayHandler>
inline void dispatchStringifyNode(const Node& yNode, IDestination& destination, const long indent,
                                  DocumentHandler&& documentHandler, NumberHandler&& numberHandler,
                                  StringHandler&& stringHandler, BooleanHandler&& booleanHandler,
                                  NullHandler&& nullHandler, TimestampHandler&& timestampHandler,
...
inline void stringifyDocument(const Node& yNode, IDestination& destination, const long indent,
                              StringifyNodesFn&& stringifyNodes) {
  stringifyNodes(NRef<Document>(yNode)[0], destination, indent);
}

template <typename WriteFn>
inline void addDelimited(IDestination& destination, const char open, const char close,
                         const std::size_t count, const std::string_view separator,
...
inline void addBooleanLiteral(IDestination& destination, const bool value,
                              const std::string_view trueText, const std::string_view falseText) {
