...
class XML_Stringify final : public IStringify {
 public:
  explicit XML_Stringify(
      std::unique_ptr<ITranslator> translator = std::make_unique<XML_Translator>()) {
 ...
 private:
  static void stringifyNodes(const Node& yNode, IDestination& destination, const long indent) {
    stringify_detail::dispatchStringifyNode(
        ...
        [](const Node& yNode, IDestination& destination) { stringifyArray(yNode, destination); });
  }
