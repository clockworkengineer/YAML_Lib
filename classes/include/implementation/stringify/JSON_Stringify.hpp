...
class JSON_Stringify final : public IStringify {
 public:
  explicit JSON_Stringify(
      std::unique_ptr<ITranslator> translator = std::make_unique<Default_Translator>()) {
 ...
 private:
  static void stringifyNodes(const Node& yNode, IDestination& destination,
    ...
    stringify_detail::dispatchStringifyNode(
        ...
        [](const Node& yNode, IDestination& destination) { stringifyNumber(yNode, destination); },
        [](const Node& yNode, IDestination& destination) { stringifyString(yNode, destination); },
        [](const Node& yNode, IDestination& destination) { stringifyBoolean(yNode, destination); },
        [](const Node& yNode, IDestination& destination) { stringifyNull(yNode, destination); },
        ...
        [](const Node& yNode, IDestination& destination) { stringifyAray(yNode, destination); });
  }
  ...
  static void stringifyDocument(const Node& yNode, IDestination& destination, const long indent) {
  ...
  static void stringifyBoolean(const Node& yNode, IDestination& destination) {
    stringify_detail::addBooleanLiteral(destination, NRef<Boolean>(yNode).value(), "true", "false");
  }
  static void stringifyNull([[maybe_unused]] const Node& yNode, IDestination& destination) {
  ...
  static void stringifyDictionary(const Node& yNode, IDestination& destination) {
    ...
    stringify_detail::addDelimited(
        destination, '{', '}', entries.size(), ",", [&](const std::size_t index) {
  ...
  static void stringifyAray(const Node& yNode, IDestination& destination) {
    ...
    stringify_detail::addDelimited(
        ...
        [&](const std::size_t index) { stringifyNodes(entries[index], destination, 0); });
  }
