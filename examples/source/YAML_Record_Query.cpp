...
static const std::string kDataFile = (fs::current_path() / "files" / "employees.yaml").string();
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char** argv) {
  try {
    ...
    for (std::size_t i = 0; i < employees.size(); ++i) {
      ...
      const auto salary = yl::NRef<yl::Number>(emp["salary"]).value<long long>();
    ...
    std::cout << "--- Summary ---";
    std::cout << "Active employees          : " << activeCount << " / " << total;
    ...
    std::cout << "Top earner                : " << topEarner << " at " << topSalary;
    ...
    for (std::size_t i = 0; i < employees.size(); ++i) {
      ...
      if (yl::NRef<yl::String>(emp["department"]).value() == "Engineering" &&
        ...
        std::cout << "  " << yl::NRef<yl::String>(emp["name"]).value()
                  << "  salary=" << yl::NRef<yl::Number>(emp["salary"]).value<long long>();
      }
    }
