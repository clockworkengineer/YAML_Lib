...
static void demoTimestamps() {
  ...
  std::cout << "  date_only     : " << yl::NRef<yl::Timestamp>(doc["date_only"]).value();
  std::cout << "  datetime_utc  : " << yl::NRef<yl::Timestamp>(doc["datetime_utc"]).value();
  std::cout << "  datetime_tz   : " << yl::NRef<yl::Timestamp>(doc["datetime_tz"]).value();
  const auto& events = yl::NRef<yl::Array>(doc["events"]);
  std::cout << "  events[0]     : " << yl::NRef<yl::Timestamp>(events[0]).value();
}
...
static void demoStandardTags() {
  ...
  std::cout << "  id      (str)  : " << yl::NRef<yl::String>(doc["id"]).value()
            << "  tag=" << doc["id"].getTag();
  std::cout << "  count   (int)  : " << yl::NRef<yl::Number>(doc["count"]).value<int>()
            << "  tag=" << doc["count"].getTag();
  std::cout << "  enabled (bool) : "
            << (yl::NRef<yl::Boolean>(doc["enabled"]).value() ? "true" : "false")
            << "  tag=" << doc["enabled"].getTag();
  std::cout << "  nothing (null) : " << (yl::isA<yl::Null>(doc["nothing"]) ? "null" : "not null")
...
static void demoNamedTagHandles() {
  ...
  std::cout << "  component value: " << yl::NRef<yl::String>(doc["component"]).value();
  std::cout << "  theme tag      : " << doc["theme"].getTag();
  std::cout << "  theme value    : " << yl::NRef<yl::String>(doc["theme"]).value();
}
...
static void demoAnchorsAliasesMerge() {
  ...
  const auto devPort = yl::NRef<yl::Number>(doc["development"]["port"]).value<int>();
  const auto devDb = yl::NRef<yl::String>(doc["development"]["database"]).value();
...
static void demoMultiDocument() {
  ...
  yl::BufferSource source{
      ...
      "title: Third\n"};
  yaml.parse(source);
  std::cout << "  Total documents: " << static_cast<unsigned long>(yaml.getNumberOfDocuments());
  for (std::size_t i = 0; i < yaml.getNumberOfDocuments(); ++i) {
    std::cout << "  doc[" << i
              << "] title = " << yl::NRef<yl::String>(yaml.document(i)["title"]).value();
  }
}
